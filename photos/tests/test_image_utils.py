...
class ImageOptimizerTestCase(TestCase):
    ...
    def test_optimize_image_display_with_smart_crop(self, mock_smart_crop, mock_find_focal):
        ...
        result, focal_point = ImageOptimizer.optimize_image(
            test_file, 'display', use_smart_crop=True
        )
    ...
    def test_generate_filename(self):
        ...
        self.assertEqual(
            ImageOptimizer.generate_filename('photo.png', 'original'),
            'photo.png'
        )
    ...
    def test_process_uploaded_image(self, mock_optimize):
        ...
        variants, focal_point = ImageOptimizer.process_uploaded_image(
            test_file, 'test.jpg'
        )
...
class DuplicateDetectorTestCase(TestCase):
    ...
    def test_find_duplicates_exact_match(self, mock_file_hash, mock_perceptual_hash):
        ...
        existing1 = Photo.objects.create(
            title="Photo 1",
            file_hash='hash123',
            perceptual_hash='phash1'
        )
        existing2 = Photo.objects.create(
            title="Photo 2",
            file_hash='hash456',
        ...
        result = DuplicateDetector.find_duplicates(
            test_file,
            Photo.objects.all(),
