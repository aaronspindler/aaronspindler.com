...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--album-id',
            type=int,
            help='Generate zip for a specific album ID'
        )
        parser.add_argument(
            '--album-slug',
            type=str,
            help='Generate zip for a specific album by slug'
        )
        parser.add_argument(
            '--all',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if options.get('all'):
            ...
            if use_async:
                result = regenerate_all_album_zips.delay()
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Triggered async regeneration of all album zips'
                    )
                )
            else:
                ...
                for album in albums:
                    ...
                    if result:
                        self.stdout.write(
                            self.style.SUCCESS(f'  ✓ Successfully generated zips for {album.title}')
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(f'  ✗ Failed to generate zips for {album.title}')
                        )
            ...
            try:
                ...
                if use_async:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'Triggered async zip generation for {album.title}')
                    )
                else:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Album with ID {album_id} does not exist')
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Album with slug "{slug}" does not exist')
                )
        
        else:
            self.stdout.write(
                self.style.WARNING(
                    'Please specify --album-id, --album-slug, or --all'
                )
            )