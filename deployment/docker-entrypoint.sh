...
# Skip static file verification for test environments
# Tests use a different STATIC_ROOT and don't need optimized static files
if [ "$DJANGO_SETTINGS_MODULE" = "config.settings_test" ]; then
    echo "Test environment detected - skipping static file verification"
else
