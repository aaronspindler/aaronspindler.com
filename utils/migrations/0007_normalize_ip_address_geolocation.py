...
def migrate_ip_addresses_forward(apps, schema_editor):
    ...
    print(f"{'='*70}")
    print(f"📊 Total RequestFingerprint records: {total_fingerprints}")
    print(f"📍 Unique IP addresses to migrate: {total_ips}")
    if total_ips > 0 and total_fingerprints > 0:
        reduction = ((total_fingerprints - total_ips) / total_fingerprints * 100)
        print(f"💾 Estimated storage reduction: {reduction:.1f}%")
    print(f"{'='*70}\n")

    if total_ips == 0:
        print("✅ No IP addresses to migrate")
        return

    # Step 1: Build a map of IP -> geo_data (one query instead of N queries)
    print("📋 Step 1/3: Building geo_data map...")
    ip_to_geo_data = {}
    ...
    print("\n🏗️  Step 2/3: Creating IPAddress records...")
    ip_address_objs = []
    ...
    print(f"   ✅ Created {total_ips} IPAddress records")

    # Step 3: Link RequestFingerprints to their IPAddress records
    print("\n🔗 Step 3/3: Linking RequestFingerprints to IPAddress records...")
    ...
    print(f"   ✅ Linked {linked_count} RequestFingerprint records in single query")
    ...
    if unlinked > 0:
        print(f"\n⚠️  WARNING: {unlinked} RequestFingerprint(s) were not linked!")
        print("   This may indicate data inconsistency. Migration will continue.")
    else:
        print(f"   ✅ Verification passed: All fingerprints linked")
    ...
    print(f"{'='*70}")
    print(f"📊 Created {total_ips} IPAddress records")
    print(f"🔗 Linked {linked_count} RequestFingerprint records")
    print(f"💾 Migrated geo_data for {len(ip_to_geo_data)} IP addresses")
    print(f"⏱️  Total time: {elapsed_time:.2f} seconds")
    if total_ips > 0:
        print(f"⚡ Average: {elapsed_time/total_ips*1000:.2f}ms per IP")
    print(f"{'='*70}\n")
...
def migrate_ip_addresses_backward(apps, schema_editor):
    ...
    print(f"{'='*70}")
    print(f"📊 Processing {total_fingerprints} RequestFingerprint records")
    print(f"⚠️  WARNING: This will duplicate geo_data storage")
    print(f"{'='*70}\n")

    if total_fingerprints == 0:
        print("✅ No records to reverse")
        return

    print("🔄 Copying IP addresses and geo_data back to RequestFingerprint...")
    ...
    print(f"{'='*70}")
    print(f"📊 Processed {processed} RequestFingerprint records")
    print(f"⏱️  Total time: {elapsed_time:.2f} seconds")
    print(f"{'='*70}\n")
