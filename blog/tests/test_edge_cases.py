...
class EdgeCaseTests(TestCase):
    ...
    def test_comment_at_max_length(self):
        ...
        max_content = "x" * 2000
        comment = BlogCommentFactory.create_comment(content=max_content)
    ...
    def test_comment_over_max_length_validation(self):
        ...
        comment = BlogComment(blog_template_name="test", content=over_max_content)
    ...
    def test_empty_blog_template_name(self):
        ...
        with self.assertRaises(IntegrityError):
            BlogComment.objects.create(blog_template_name=None, content="Test")

    def test_deeply_nested_comments(self):
        """Test handling of deeply nested comment threads."""
        parent = BlogCommentFactory.create_approved_comment(content="Level 0", blog_template_name="test")

        current = parent
        for i in range(1, 10):  # Create 9 levels of nesting
            current = BlogCommentFactory.create_approved_comment(
                content=f"Level {i}", blog_template_name="test", parent=current
            )
    ...
    def test_circular_reference_prevention(self):
        ...
        comment1 = BlogCommentFactory.create_comment(content="Comment 1")

        comment2 = BlogCommentFactory.create_comment(content="Comment 2", parent=comment1)
    ...
    def test_vote_on_own_comment(self):
        ...
        comment = BlogCommentFactory.create_approved_comment(content="My comment", author=self.user)
    ...
    def test_simultaneous_vote_changes(self):
        ...
        # Create initial vote
        vote = BlogCommentFactory.create_comment_vote(comment=comment, user=self.user, vote_type="upvote")
    ...
    def test_unicode_in_comments(self):
        ...
        comment = BlogCommentFactory.create_comment(content=unicode_content, author_name="José García")
        ...
        form_data.update({"content": unicode_content, "author_name": "José García"})
    ...
    def test_xss_prevention_in_content(self):
        ...
        for xss in xss_attempts:
            comment = BlogCommentFactory.create_comment(content=xss)
    ...
    def test_sql_injection_prevention(self):
        ...
        comment = BlogCommentFactory.create_comment(content=sql_injection, author_name=sql_injection)
    ...
    def test_malformed_email_normalization(self):
        ...
        for input_email, expected in test_cases:
            form_data = MockDataFactory.get_common_form_data()["comment_form"]
            form_data.update({"content": "Test", "author_email": input_email})
            form = CommentForm(data=form_data)
    ...
    def test_concurrent_comment_submission(
        self, mock_reverse, mock_get_approved, mock_request_fingerprint, mock_get_blog
    ):
    ...
    def test_invalid_vote_type(self):
        ...
        response = self.client.post(f"/comment/{comment.id}/vote/", {"vote_type": "superupvote"})  # Invalid
