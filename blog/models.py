...
class BlogComment(models.Model):
    ...
    blog_category = models.CharField(max_length=50, help_text="Category of the blog post (e.g., 'tech', 'personal')")
    ...
    author_name = models.CharField(max_length=100, blank=True, help_text="Name for anonymous commenters")
    author_email = models.EmailField(blank=True, help_text="Email for anonymous commenters (not displayed publicly)")
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of commenter for spam detection"
    )
    ...
    def __str__(self):
        ...
        blog_display = (
            f"{self.blog_category}/{self.blog_template_name}" if self.blog_category else self.blog_template_name
        )
    ...
    def get_approved_comments(cls, template_name, category=None):
        ...
        replies_prefetch = Prefetch(
            ...
            .prefetch_related(
                Prefetch(
                    "replies",
                    queryset=cls.objects.filter(status="approved").select_related("author"),
                )
            ),
        )
...
class CommentVote(models.Model):
    ...
    comment = models.ForeignKey(BlogComment, on_delete=models.CASCADE, related_name="votes")
