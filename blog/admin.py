...
class TagAdmin(admin.ModelAdmin):
    """Admin interface for managing content tags."""
    
    list_display = ['name_badge', 'slug', 'usage_count', 'created_at']
    ...
    def name_badge(self, obj):
        """Display tag name with colored badge."""
        return format_html(
    ...
    def usage_count(self, obj):
        """Display count of blog posts using this tag."""
...
class BlogPostTagAdmin(admin.ModelAdmin):
    """Admin interface for blog post tag associations."""
    
    list_display = ['get_blog_post', 'tag_badge', 'created_at']
    list_filter = ['tag', 'blog_category', 'created_at']
    search_fields = ['blog_template_name', 'blog_category', 'tag__name']
    ...
    def tag_badge(self, obj):
        """Display tag with colored badge."""
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 8px; border-radius: 3px; font-weight: bold;">{}</span>',
            obj.tag.color, obj.tag.name
        )
    tag_badge.short_description = 'Tag'
    tag_badge.admin_order_field = 'tag__name'


class BlogPostSeriesMembershipInline(admin.TabularInline):
    """Inline for managing posts within a series."""
    model = BlogPostSeriesMembership
    extra = 1
    fields = ['part_number', 'blog_category', 'blog_template_name']
    ordering = ['part_number']


@admin.register(BlogPostSeries)
class BlogPostSeriesAdmin(admin.ModelAdmin):
    """Admin interface for blog post series."""
    
    list_display = ['name', 'slug', 'posts_count', 'created_at', 'updated_at']
    search_fields = ['name', 'slug', 'description']
    prepopulated_fields = {'slug': ('name',)}
    list_filter = ['created_at', 'updated_at']
    date_hierarchy = 'created_at'
    inlines = [BlogPostSeriesMembershipInline]
    ...
    def posts_count(self, obj):
        """Display count of posts in this series."""
        count = obj.posts.count()
        if count > 0:
            return format_html('<strong>{}</strong> parts', count)
        return '0 parts'
    posts_count.short_description = 'Posts'


@admin.register(BlogPostSeriesMembership)
class BlogPostSeriesMembershipAdmin(admin.ModelAdmin):
    """Admin interface for series memberships."""
    
    list_display = ['series', 'part_number', 'get_blog_post', 'created_at']
    list_filter = ['series', 'blog_category', 'created_at']
    search_fields = ['blog_template_name', 'blog_category', 'series__name']
    autocomplete_fields = ['series']
    date_hierarchy = 'created_at'
    
    fieldsets = (
        ('Series', {
            'fields': ('series', 'part_number')
        }),
        ('Blog Post', {
            'fields': ('blog_category', 'blog_template_name')
        }),
        ('Metadata', {
            'fields': ('created_at',),
            'classes': ('collapse',)
