...
test-up:
	...
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test up -d
...
test-run:
	...
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test run --rm test_runner
...
test-run-app:
	...
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test run --rm test_runner python manage.py test $(APP) --settings=config.settings_test --parallel --keepdb
...
test-run-specific:
	...
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test run --rm test_runner python manage.py test $(TEST) --settings=config.settings_test --keepdb

.PHONY: test-shell
test-shell:
	@echo "Opening shell in test container..."
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test run --rm web bash
...
test-coverage:
	...
	docker compose -f deployment/docker-compose.yml --env-file deployment/env.test run --rm test_runner sh -c "coverage run --source='.' manage.py test --settings=config.settings_test && coverage report && coverage html"
