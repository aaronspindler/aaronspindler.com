...
test-build:
	...
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml build

.PHONY: test-up
test-up:
	@echo "Starting test environment..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test up -d
...
test-run:
	...
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test run --rm test_runner
...
test-run-app:
	...
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test run --rm test_runner python manage.py test $(APP) --settings=config.settings_test --parallel --keepdb
...
test-run-specific:
	...
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test run --rm test_runner python manage.py test $(TEST) --settings=config.settings_test --keepdb

.PHONY: test-shell
test-shell:
	@echo "Opening shell in test container..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test run --rm web bash

.PHONY: test-down
test-down:
	@echo "Stopping test environment..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml down
	@echo "Test environment stopped!"

.PHONY: test-clean
test-clean:
	@echo "Cleaning test environment (removing volumes)..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml down -v
	@echo "Test environment cleaned!"

.PHONY: test-logs
test-logs:
	@echo "Showing test environment logs..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml logs -f
...
test-logs-service:
	...
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml logs -f $(SERVICE)


.PHONY: test-coverage
test-coverage:
	@echo "Running tests with coverage in Docker..."
	docker-compose -f deployment/docker-compose.infra.yml -f deployment/docker-compose.app.yml --env-file deployment/env.test run --rm test_runner sh -c "coverage run --source='.' manage.py test --settings=config.settings_test && coverage report && coverage html"
