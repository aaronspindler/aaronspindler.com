...
class Command(BaseCommand):
    ...
    def _purge_css(self, base_dir, input_path, static_dir):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"  ✓ Purged: {purged_size:.1f}KB (removed {reduction:.1f}%)"))
            return output_path
...
""" % (
    ...
    def _final_minification(self, input_path, static_dir):
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, check=True, cwd=settings.BASE_DIR)
    ...
    def _parse_css(self, content):
        ...
        for mq in media_queries:
            ...
            if media_match:
                ...
                rules.append({"type": "media", "condition": condition, "rules": inner_rules})
    ...
    def _parse_regular_rules(self, content):
        ...
        for selector, properties in matches:
            ...
            if selector:
                ...
                if props:
                    rules.append({"type": "rule", "selector": selector, "properties": props})
    ...
    def _merge_duplicate_selectors(self, rules):
        ...
        for selector, props_list in selector_map.items():
            ...
            merged.append({"type": "rule", "selector": selector, "properties": merged_props})
    ...
    def _consolidate_media_queries(self, rules):
        ...
        for condition, rules_lists in media_map.items():
            ...
            consolidated.append({"type": "media", "condition": condition, "rules": merged_rules})
    ...
    def _print_optimization_summary(self, static_dir, css_files):
        ...
        for css_file in css_files:
            ...
            if os.path.exists(file_path):
                ...
                if original_size > 0:
                    ...
                    self.stdout.write(
                        f"  {css_file:20s}: {original_size:>6d}B → {new_size:>6d}B " f"(-{reduction:>5.1f}%)"
                    )
        ...
        if total_original > 0:
            ...
            self.stdout.write("-" * 60)
            self.stdout.write(
                f'  {"TOTAL":20s}: {total_original:>6d}B → {total_new:>6d}B ' f"(-{total_reduction:>5.1f}%)"
            )
